...
class ViewportControllerImpl implements ViewportController {
  ...
  readonly #regionTransform: TransformTool;
  ...
  constructor(options: ViewportControllerOptions) {
    ...
    this.#regionTransform = createTransformTool({ host, editor: this.#editor });
  ...
  transformPreviewRotate(axis: ShapeAxis): ToolActionResult {
    return this.#report(this.#regionTransform.previewRotate(axis));
  }

  transformPreviewMirror(axis: ShapeAxis): ToolActionResult {
    return this.#report(this.#regionTransform.previewMirror(axis));
  }

  transformPreviewDelete(): ToolActionResult {
    return this.#report(this.#regionTransform.previewDelete());
  }

  transformApply(): ToolActionResult {
    return this.#report(this.#regionTransform.applyPending());
  }

  transformCancel(): void {
    this.#regionTransform.cancelPending();
  }
  ...
  #activeTool(): Tool | undefined {
    ...
    switch (tool) {
      ...
      case "transform":
        return this.#regionTransform;
    }
  }

  /** Host services for the headless transform gizmo tool (plan S7.8). */
  #makeTransformHost(): NodeTransformToolHost {
