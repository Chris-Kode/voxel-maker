...
function openFixture(composition: DesktopComposition, filledB = false): void {
...
describe("desktop transform move", () => {
  it("moves the selected node geometry as one labeled, undoable transaction", () => {
  ...
  it("previews exact overwrite counts when the destination overlaps content", () => {
    const composition = createDesktopComposition({
      storage: new MemoryProjectStorage(),
      picker: createFakePicker(),
    });
...
describe("desktop transform copy", () => {
  it("copies the selection into box B's empty volume area in one transaction", () => {
    const composition = createDesktopComposition({
      storage: new MemoryProjectStorage(),
      picker: createFakePicker(),
    });
    openFixture(composition, true);
    frameFront(composition);
    const start = scanPick(composition, (hit) => hit.nodeId === CHILD);
    if (start === undefined) throw new Error("box A must be pickable");
    composition.viewport.selectAt(start.x, start.y);
...
describe("desktop transform rotate", () => {
  it("previews, applies, and undoes a 90-degree rotation around z", () => {
    const composition = createDesktopComposition({
      storage: new MemoryProjectStorage(),
      picker: createFakePicker(),
    });
  ...
  it("applies 90-degree rotations around every axis and redoes them", () => {
    const composition = createDesktopComposition({
      storage: new MemoryProjectStorage(),
      picker: createFakePicker(),
    });
  ...
  it("cancels a pending preview on Escape with zero side effects", () => {
    const composition = createDesktopComposition({
      storage: new MemoryProjectStorage(),
      picker: createFakePicker(),
    });
  ...
  it("falls back to the always-exact 180-degree step for parity mismatches", () => {
    const composition = createDesktopComposition({
      storage: new MemoryProjectStorage(),
      picker: createFakePicker(),
    });
...
describe("desktop transform mirror", () => {
  it("previews and applies a mirror as one labeled, undoable transaction", () => {
    const composition = createDesktopComposition({
      storage: new MemoryProjectStorage(),
      picker: createFakePicker(),
    });
...
describe("desktop transform delete", () => {
  it("previews and applies a delete as one labeled, undoable transaction", () => {
    const composition = createDesktopComposition({
      storage: new MemoryProjectStorage(),
      picker: createFakePicker(),
    });
