...
function openFixture(harness: Harness): void {
  const result = harness.composition.fileService.openLoadedProject(
...
describe("material panel controller", () => {
  it("lists materials with live usage counts when a document opens", () => {
  ...
  it("createMaterial commits one transaction and activates the new material", () => {
    const harness = createHarness();
    openFixture(harness);
  ...
  it("updateMaterial edits every canonical field through commands", () => {
    const harness = createHarness();
    openFixture(harness);
    const revisionBefore =
      harness.composition.session.current?.store.revision ?? 0;
  ...
  it("updateMaterial rejects a missing material without committing", () => {
    const harness = createHarness();
    openFixture(harness);
    const revisionBefore =
      harness.composition.session.current?.store.revision ?? 0;
  ...
  it("deleteMaterial removes an unreferenced material directly", () => {
    const harness = createHarness();
    openFixture(harness);
    const revisionBefore =
      harness.composition.session.current?.store.revision ?? 0;
  ...
  it("referenced deletion requires a valid reassignment", () => {
    const harness = createHarness();
    openFixture(harness);
  ...
  it("undo and redo preserve material semantics and voxel assignments", () => {
    const harness = createHarness();
    openFixture(harness);
  ...
  it("never reuses a material id while history can mention it", () => {
    const harness = createHarness();
    openFixture(harness);
  ...
  it("rejects a malformed color update with a structured error", () => {
    const harness = createHarness();
    openFixture(harness);
    const revisionBefore =
      harness.composition.session.current?.store.revision ?? 0;
  ...
  it("history flags track undo/redo availability", () => {
    const harness = createHarness();
    openFixture(harness);
  ...
  it("save and reload preserve material semantics and voxel assignments", async () => {
    ...
    const opened = reloaded.composition.fileService.openLoadedProject(
      "reload.vxl",
      bytes,
    );
  ...
  it("tracks external commits and resets on lifecycle replacement", () => {
    const harness = createHarness();
    openFixture(harness);
    ...
    const closed = harness.composition.fileService.closeProject();
  ...
  it("paintWith switches the runtime paint material", () => {
    const harness = createHarness();
    openFixture(harness);
  ...
  it("refuses creation at the document material limit", () => {
    const harness = createHarness();
    const document = createDocument({
