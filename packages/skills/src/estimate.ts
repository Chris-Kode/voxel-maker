 ...
 * restricted to the command types the v1 generator factory can emit; the
 * lifecycle tests pin the two implementations together by asserting that
 * a proposal's preflight total equals the preview session's enforced
 * cumulative estimate after staging. Malformed payloads estimate 0 and
 * are rejected by command validation instead.
 */
