...
Normal editing, import into an open document, AI Apply, undo, and redo declare an expected base revision. Stale work fails before staging. Identical committed transaction replay follows the idempotency policy; reusing an identifier with different canonical bytes fails. A committed command id is the unique identity of one committed transaction: the bus retains the command ids that ran in its transaction stream for the open session and every retained recovery frame, and a new normal commit that reuses one fails atomically with `DUPLICATE_COMMAND_ID` before staging. Undo, redo, and gesture-cancel rollback replay stored commands and derived inverses and are exempt from the reuse check; recovery replays recorded frames on a fresh bus.
...
| `evaluation` | Fixed geometry evaluation harness: deterministic scenarios, recorded traces, scoring, version recording, promotion gates | Agent loop/tool contracts, the standard preview renderer, and semantic read modules |
...
Every version 1 persistent edit command is undoable. Undo executes stored inverses in reverse command order as one special transaction. Redo replays an owned deep-frozen snapshot of the originally committed commands (parsed, bounded payloads), so mutating a submitted command object after commit can never change what redo replays (issue #113). A new normal commit clears redo. Lifecycle replacement resets history. Coalescing requires an explicit deterministic key and compatible affected resources on the latest unsealed entry; it combines history presentation, never semantic commits or revisions, and seals on gesture end, incompatible or intervening commit, undo/redo, lifecycle replacement, or failure. Idempotency records remain available for the open session and retained recovery horizon, as do the command ids that ran in the session's transaction stream: a new normal commit reusing a committed command id fails atomically with `DUPLICATE_COMMAND_ID` (issue #115).
...
The desktop workflow (plan S7.16, ticket #22, `docs/editor/project-lifecycle-v1.md`) drives new, open, save, save-as, recent-project, replace, close, and recovery through the lifecycle coordinator and the injected scoped storage ports. Each open document owns a save coordinator, a recovery journal, a shared snapshot-write gate, and a debounced autosave binding that are disposed and rebound on every lifecycle replacement; committed transactions reach the journal through the session's bus hooks. Every snapshot replacement for the project path — manual save, autosave, compaction, and save-as reassociation — goes through the shared gate, which serializes writes per path and fences stale captured revisions so an older save can never overwrite a newer compacted snapshot or its journal anchor (ticket #51). Dirty-close, overwrite, and recovery decisions go through an injected prompt service; pending-save, stale-completion, and degraded-recovery states are exposed on the workflow status so the shell can render them. Recent projects are a bounded, scoped list (localStorage in the browser build, the app-config-dir JSON file in the Tauri shell).
