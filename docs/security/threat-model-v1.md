...
| 8 | Credentials | Keychain service/account/value | Forged service/account addressing other keychain entries, wildcard/oversized entries, secret leakage into logs | Rust pins the service to `voxel-maker:provider` (not an IPC parameter) and allowlists provider accounts (`openai` in v1); value ≤ 16 KiB; keys live only in the OS keychain; values never logged | `lib.rs` keychain-scope tests (issue #95), `credentials.test.ts`, `check-native-capabilities.mjs` |
